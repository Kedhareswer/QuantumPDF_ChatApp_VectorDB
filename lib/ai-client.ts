...
export class AIClient {
  ...
  async generateText(messages: ChatMessage[]): Promise<string> {
    try {
      ...
      switch (this.config.provider) {
        ...
        case "google":
          return await this.generateGoogleText(messages)
        case "vertex":
  ...
  private async generateOpenRouterText(messages: ChatMessage[]): Promise<string> {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        console.warn("Could not parse OpenRouter error response as JSON")
      }
  ...
  private async testCohereConnection(): Promise<boolean> {
    try {
      await this.generateCohereText([{ role: "user", content: "test" }])
      return true
    } catch {
      return false
    }
  ...
  // Google AI Studio implementations
  private async generateGoogleText(messages: ChatMessage[]): Promise<string> {
  ...
  private async testGoogleConnection(): Promise<boolean> {
    try {
      await this.generateGoogleText([{ role: "user", content: "test" }])
      return true
    } catch {
      return false
    }
  ...
  private async generateVertexEmbedding(text: string): Promise<number[]> {
    // Note: This is a simplified implementation. Real Vertex AI requires OAuth2 authentication
    const baseUrl = this.config.baseUrl || "https://us-central1-aiplatform.googleapis.com/v1"

    const response = await fetch(
      `${baseUrl}/projects/YOUR_PROJECT/locations/us-central1/publishers/google/models/textembedding-gecko:predict`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${this.config.apiKey}`,
  ...
  private async generateVertexText(messages: ChatMessage[]): Promise<string> {
    ...
    const response = await fetch(
      `${baseUrl}/projects/YOUR_PROJECT/locations/us-central1/publishers/google/models/${this.config.model}:generateContent`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${this.config.apiKey}`,
  ...
  private async testTogetherConnection(): Promise<boolean> {
    try {
      await this.generateTogetherText([{ role: "user", content: "test" }])
      return true
    } catch {
      return false
    }
