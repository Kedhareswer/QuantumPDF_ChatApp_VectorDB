...
- **Hugging Face Connectivity Issues**:
    - **API Key (Server-Side Critical)**:
        - **Server-Side Environment Variable is Key**: For Hugging Face integration to work, the `HUGGINGFACE_API_KEY` **must be set as an environment variable on the server where your Next.js application (including its API routes) is running.** The application logs whether this key is found when API routes are accessed. Check server logs for messages like 'Hugging Face API Key found: Yes/No'.
        - **UI-Provided Key vs. Environment Variable**: While you might provide a Hugging Face API key through the application's UI (e.g., in API configuration settings), the system's backend routes (which handle Hugging Face requests) will primarily use the `HUGGINGFACE_API_KEY` found in the server's environment variables. If this server-side environment variable is missing or incorrect, Hugging Face operations will fail, even if a key is entered in the UI.
        - **RAG Engine Initialization**: Specifically, if the RAG Engine fails to initialize with Hugging Face as the provider (showing an error like 'Failed to connect to AI provider'), the most common cause is that the server-side API routes (e.g., `/api/test/huggingface`) are unable to authenticate with Hugging Face due to the `HUGGINGFACE_API_KEY` not being correctly set up in the server's environment.
